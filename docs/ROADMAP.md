...
- **The bloat + the blast primitive** (2026-08-12). A red ooze that arms inside
  ...
  not. The blast hits **mobs too**, so it's the first enemy that's more useful
  as a tool than as a kill. Underneath: one `blast` action + `radial-blast.ts`,
  replacing three half-blind copies of the same radius loop (`aoe` = player only,
  rite nova = mobs only, bleed chain = a third). **Tuning trap caught in
  playtest:** shipped at 4 damage against a 5 HP player and deleted a healthy
  player in one beat; now 3, with a test asserting it can never one-shot from
  full. `?scenario=bloat-lab`, or the Bloat test chamber.
...
| ~~#141~~ | ~~Attack motion tracks~~ **SHIPPED 2026-08-12** — `Ability.motion`, a root-motion span authored in normalized window time and applied along COMMITTED FACING (never homing — you still beat it by not being on the line). Every melee-opening ability picks up a default step-in anchored to its own contact frame, so a stoneguard's heave and a skirmisher's jab both lean in the same beat before landing. Rooted mobs (moveSpeed 0) are excluded by rule, not by special case. **Open risk:** windup CREEP and step-in now stack — creep was the workaround for attacks that couldn't travel, and may want cutting once the step-in is felt. | Josh 2026-08-12, the big one in this block. A `motion` track on the ability/verb, resolved by the same step-trigger clock that already drives `melee`/`aoe`. Vocabulary to build against: **step-in** (~0.8m on the strike frame — should become the DEFAULT on baseline melee, it's the cheapest cure), **coil-dash** (readable crouch → 3-4m dash → long punishable recovery; `skirmisher` already does this via the `dash` action and is the proof), **advancing combo** (each swing steps, so backpedalling doesn't escape), **hit-and-run** (strike then immediate backstep out of counter range), **orbit** (refuses to commit while a packmate does — `intent.ts` already has `circle`). **Trap:** root motion must go through `walkable.clampMoveInto`/nav like `dash` does, or mobs will phase through geometry. |
...
| **#134** | Finisher ceremony | Risk: an animation lock in a swarm is how you die unfairly. Fast **and** i-framed. **Shape borrowed from WWM (`COMBAT-RESEARCH.md` §1c):** make it a CONTEXTUAL TIMED PROMPT rather than a dedicated input — it costs no button because it only exists when it's legal, and we already have tap-target + interact to hang it on. Give the prompt a visible countdown and let greed pay: correct play should be to keep hitting the staggered enemy and fire the finisher just before the window closes, which turns a button-prompt into a small risk curve. Ours today is a charged heavy with no prompt, no timer and no decision. |
...
7. **A detector that finds nothing looks exactly like a codebase with nothing to
   ...
   under `a`/`b`.)
8. **Changing a DEFAULT ships nothing to anyone who already has a save.** An
   explicit stored value beats a changed default forever, so the feature lands
   for new installs only and the people playing daily never see it. If the new
   default matters, it needs a one-time marker in `load()`. (New, 2026-08-12:
   the dodge button was the default for six days while Josh — asking for a dodge
   button — was pinned to the old gesture. Third instance, after portal culling
   and the shadow rebaseline.)
