...
export interface Nav {
  /** Compass toward the next path waypoint from `from` toward `to`, routing around
   *  walls/pillars and THROUGH framed openings (the NavGrid funnels the path to gate
   *  centres). null if no route (target unreachable / walled off). */
...
export function makeNav(grid: NavGrid): Nav {
  ...
  return {
    dirToward(from: Vec2, to: Vec2): Direction8 | null {
      // radius = player, so the grid excludes gaps the player can't fit and admits
