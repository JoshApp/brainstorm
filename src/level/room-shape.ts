...
export type Archetype = 'hall' | 'chamber' | 'apse' | 'ell' | 'cross' | 'notched';

export const ARCHETYPES: readonly Archetype[] = ['hall', 'chamber', 'apse', 'ell', 'cross', 'notched'];
...
export function generateRoomShape(kind: Archetype, opts: ShapeOpts): Poly {
  ...
  return cut > 0 ? chamfer(poly, cut, () => rand() < 0.75) : poly;
}
