...
function spanGeometry(
  ...
  collapse = false,
  /** Cap the UNDERSIDE too. A wall stands on the floor and needs none; a lintel
   *  stands over a doorway and shows its belly to anyone walking through. */
  capBottom = false,
  ...
  // Soffit — the top cap's winding reversed, so it faces DOWN.
  if (capBottom) {
    out.push(quad(
      [s.oa[0], lo, s.oa[1]], [s.ob[0], lo, s.ob[1]],
      [s.b[0], lo, s.b[1]], [s.a[0], lo, s.a[1]],
    ));
  }
