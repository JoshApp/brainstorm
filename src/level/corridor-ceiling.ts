...
// ── AND THE CEILING IS JUST THE FLOOR, PLUS THE HEIGHT ───────────────────────
//
...
//
// It does, and there is no code here for it any more, which is the point. Two passes tried
// to fix a floor defect at the ceiling: first by raking it into one straight plane, which
// removed the creases by refusing to follow the floor at all and paid in headroom; then by
// rounding the CEILING's knees while the floor's stayed sharp, which the suite caught within
// a minute — the two profiles were no longer parallel and a flight pinched to 2.54m under a
// 3.00m section.
//
// The corners were always in the FLOOR: level landing, a hard angle into the rake, a hard
// angle back. So the easing moved there (level/corridor-stair.ts `roundedRampProfile`, used
// by the elevation field for every ramp), and the ceiling went back to `groundYAt + H`:
// exactly parallel to the ground, headroom constant everywhere, and creaseless because there
// is no longer a crease to inherit.
