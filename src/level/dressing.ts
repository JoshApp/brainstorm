...
export const DRESSING: Record<string, DressingEntry> = {
  ...
  'shell-coursing': {
    ...
    note: 'PER-COURSE DEPTH in the wall face — every course of every wall pushed '
      ...
      + 'ALSO GATES THE CORRIDOR WAVE (level/geometry-prims.ts). Rect walls — which '
      + 'is what a corridor is — are built by a different function that put a smooth '
      + '0.045m undulation on the face, so the dungeon had flat walls in its rooms '
      + 'and undulating ones in its corridors. One idea, two producers, one of them '
      + 'switched: the same shape of miss as buttress/pilaster and wall-pile/collapse, '
      + 'and reason enough on its own.\n'
      + 'It is NOT claimed to fix the report it was found chasing (Josh: "the corridor '
      + 'two faces inside dont use the pom raymarch there or they do but its broken, '
      + 'also their texture is distorted"). That guess was that displacing a face '
      + 'swings the shader\'s projection axis; measured, the tilt is 6.2 degrees, not '
      + 'the ~27 I estimated. See tests/wall-plane-flatness.test.ts. The report is '
      + 'still open.\n'
